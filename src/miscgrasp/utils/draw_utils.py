...
from src.miscgrasp.utils.base_utils import compute_relative_transformation, compute_F
import numpy as np
import cv2
...
def draw_gripper(img, R, t, K, width, thickness=2, dist=None):
    ...
    dist = np.zeros(4, dtype=float) if dist is None else dist
    points = np.float32(
        [[0, -width / 2, 0], [0, -width / 2, 0.05], [0, width / 2, 0], [0, width / 2, 0.05], [0, 0, -0.05],
         [0, 0, 0]]).reshape(-1, 3)
    axis_points, _ = cv2.projectPoints(points, rotation_vec, t, K, dist)
    axis_points = axis_points.astype(int)

    # ravel()用来将数组维度拉成一维数组，cv2.projectPoints生成的点为（n，1，2）格式
...
def draw_gripper_o3d(R, t, width, score=1, color=None):
    ...
    **Output:**
    ...
    # print(vertices.shape, R.shape, np.dot(R, vertices.T).shape)
