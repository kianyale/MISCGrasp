...
from src.miscgrasp.utils.base_utils import color_map_forward, pad_img_end
...
def random_crop(ref_imgs_info, que_imgs_info, target_size):
    ...
    def crop_imgs_info(imgs_info):
        ...
        h_init = center_h - out_h // 2  # 裁减后图片的中心相对于原图片坐标中心的位置
...
def random_flip(ref_imgs_info, que_imgs_info):
    def flip(tensor):
        ...
        tensor = np.ascontiguousarray(tensor.transpose([0, 3, 1, 2]))  # ascontiguousarray函数将一个内存不连续存储的数组转换为
        # 内存连续存储的数组，使得运行速度更快
        return tensor
...
def pad_imgs_info(ref_imgs_info, pad_interval):  # pad_interval = 32
    ...
    if ph != 0 or pw != 0:  # h = 512, w = 288, pad_interval = 32时，刚好均为0
...
def build_imgs_info(database, ref_ids, pad_interval=-1, is_aligned=True, align_depth_range=False, has_mask=True,
                    has_depth=True, replace_none_depth=False):
    if not is_aligned:
        assert has_depth
        rfn = len(ref_ids)
        ...
        ref_imgs = color_map_forward(np.asarray([database.get_image(ref_id) for ref_id in ref_ids])).transpose(
            [0, 3, 1, 2])  # [b, h, w, c] -> [b, c, h, w] : [6, 3, 288, 512]
        if has_mask:
        ...
        if has_depth:
            ...
            ref_depths = np.asarray(ref_depths, dtype=np.float32)[:, None, :, :]  # [6, 1, 288, 512]
        else:
            ref_depths = None

    ref_poses = np.asarray([database.get_pose(ref_id) for ref_id in ref_ids], dtype=np.float32)  # [6, 3, 4]
    ref_Ks = np.asarray([database.get_K(ref_id) for ref_id in ref_ids], dtype=np.float32)  # [6, 3, 3]
    ref_depth_range = np.asarray([database.get_depth_range(ref_id) for ref_id in ref_ids], dtype=np.float32)  # [6, 2]
    if align_depth_range:
        ref_depth_range[:, 0] = np.min(ref_depth_range[:, 0])
        ref_depth_range[:, 1] = np.max(ref_depth_range[:, 1])
    ref_imgs_info = {'imgs': ref_imgs, 'poses': ref_poses, 'Ks': ref_Ks, 'depth_range': ref_depth_range,
                     'masks': ref_masks, 'bbox3d': database.get_bbox3d()}  # reg_imgs_info['bbox3d']等于直接调用get_bbox3d()
    if has_depth:
