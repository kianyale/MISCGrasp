...
from src.miscgrasp.utils.base_utils import color_map_forward
from src.miscgrasp.utils.draw_utils import extract_surface_points_from_volume
...
def sim_grasp(database, alp_vol, qual_vol, rot_vol, width_vol, top_k=10):
    from src.miscgrasp.utils.grasp_utils import select, process
...
def run_real(run_id, model, images: list, extrinsics: list, intrinsic, save_img=True):
    ...
    t0 = time.time()
    tsdf_vol, qual_vol, rot_vol, width_vol = model(images, extrinsics, intrinsics, depth_range=depth_range,
                                                   bbox3d=bbox3d, que_id=3)
    t = time.time() - t0
