...
class LearningRateManager(abc.ABC):
    ...
    def construct_optimizer(self, optimizer, network):  # , trainloss):
        # may specify different lr for different parts
        # use group to set learning rate
        # paras = [{'params': network.parameters(), 
        #           'params': trainloss[-1].parameters()
        #           }]
        paras = network.parameters()
        return optimizer(paras, lr=1e-3)  #, weight_decay=1e-4)
...
class ExpDecayLR(LearningRateManager):
    ...
    def __call__(self, optimizer, step, *args, **kwargs):
        lr = max(self.lr_init * (self.decay_rate ** (step // self.decay_step)), self.lr_min)  # NOTE: here is exact division
