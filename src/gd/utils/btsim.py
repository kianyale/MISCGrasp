...
class Body(object):
    ...
    # The following is specifically designed for grippers
    def configure(self, n_links_before):
        # Set friction coefficients for gripper fingers
        for i in range(n_links_before, self.p.getNumJoints(self.uid)):
            self.p.changeDynamics(self.uid, i, lateralFriction=1.0, spinningFriction=1.0,
...
class Link(object):
    ...
    def get_pose(self):
        ...
        return Transform(Rotation.from_quat(ori), list(pos))
...
class Camera(object):
    ...
    def render(self, extrinsic):
        ...
        gl_view_matrix[2, :] *= -1  # flip the Z axis  NOTE: this line does flip the Z axix
