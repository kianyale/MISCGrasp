...
class GripperEZGripper(GripperBase):
    """
    Initialization of EZGgripper gripper
    """
    name = 'ezgripper'
    def __init__(self, world, gripper_size=1.):
        self.world = world
        self._gripper_size = gripper_size
        self.urdf_path = Path("/media/yons/6d379145-c1d8-430f-9056-7777219c83a8/MISCGrasp/data/assets/gripper/ezgripper/combined_mount_ezgripper.urdf")
        self.max_opening_width = 0.112 * self._gripper_size
        self.finger_depth = 0.095 * self._gripper_size
        self.finger_depth_init = 0.082 * self._gripper_size
        ...
        self.T_tcp_trlink = Transform(Rotation.from_matrix(np.array([[0., 0., -1.],
                                                                     [0., 1., 0.],
                                                                     [1., 0., 0.]])),
                                      np.array([0., 0., -0.0855]) * self._gripper_size)
        ...
        self._grasp_force = 60
        self._grasp_speed = 3
        self.inherent_link_len = 0.02688 * self._gripper_size
    ...
    def reset(self, T_world_tcp):
        ...
        self.link1 = self.body.links["left_ezgripper_finger_L2_1"]
        self.link2 = self.body.links["left_ezgripper_finger_L2_2"]

        self.traced_link = self.body.links["world"]
...
if __name__ == '__main__':
    ...
    gripper = GripperEZGripper(world)

    # T_world_grasp = Transform(Rotation.from_euler('XYZ', [-np.pi, np.pi / 2, 0.]), [0., 0.112 / 2, 0.])
    T_world_grasp = Transform(Rotation.from_euler('XYZ', [-np.pi, 0., 0.]), [0., 0., 0.])
    ...
    T_world_trlink = gripper.body.links['world'].get_pose()
