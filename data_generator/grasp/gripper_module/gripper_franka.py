...
class GripperFranka(GripperBase):
    """
    Initialization of Franka 2 finger gripper
    """
    name = 'franka'
    def __init__(self, world, gripper_size=1.):
        ...
        self.urdf_path = Path("/media/yons/6d379145-c1d8-430f-9056-7777219c83a8/MISCGrasp/data/assets/gripper/franka/combined_mount_franka.urdf")
        self.max_opening_width = 0.08 * self._gripper_size
        self.finger_depth = 0.05 * self._gripper_size
        ...
        self.T_tcp_trlink = Transform(Rotation.from_matrix(np.array([[1., 0., 0.],
                                                                     [0., 1., 0.],
                                                                     [0., 0., 1.]])),
                                      np.array([0., 0.0015, -0.0374]) * self._gripper_size)
    ...
    def reset(self, T_world_tcp):
        ...
        self.traced_link = self.body.links["panda_hand"]
...
if __name__ == '__main__':
    ...
    gripper = GripperFranka(world)
    T_world_grasp = Transform(Rotation.from_euler('XYZ', [-np.pi, np.pi / 2., 0.]),
                              [0., 0.04, 0.])#gripper.finger_depth_close])
    # T_world_grasp = Transform(Rotation.from_euler('XYZ', [0., 0., 0.]),
    ...
    print(gripper.read())
    T_world_trlink = gripper.body.links['panda_hand'].get_pose()
