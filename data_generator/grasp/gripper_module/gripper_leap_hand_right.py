...
class GripperLeapHandRight(GripperBase):
    """
    Initialization of Leap Right Hand gripper
    """
    ...
    def __init__(self, world, gripper_size=1.):
        ...
        # self.urdf_path = Path("/media/yons/6d379145-c1d8-430f-9056-7777219c83a8/MISCGrasp/data/assets/gripper/leap_hand_right/combined_mount_leap_hand_right"
        #                       ".urdf")
        self.urdf_path = Path("/media/yons/6d379145-c1d8-430f-9056-7777219c83a8/MISCGrasp/data/assets/gripper/leap_hand_right2/combined_mount_leap_hand_right2.urdf")
        self.max_opening_width = 0.124 * self._gripper_size
        # 'left' and 'right' are defined relative to the x-axis of the gripper's reference frame.
        self.finger_open_distance_right = 0.064 * self._gripper_size  # single-finger side
        self.finger_open_distance_left = 0.06 * self._gripper_size  # two-finger side
        self.half_gap_2f = 0.045 * self._gripper_size  # Distance from one of the two fingers to the y-axis (half of
        # the gap between the two fingers)

        self.finger_depth_init = 0.105 * self._gripper_size
        self.finger_depth = 0.111 * self._gripper_size
        ...
        self.T_tcp_trlink = Transform(Rotation.from_matrix(np.array([[0., -1., 0.],
                                                                     [-1., 0., 0.],
                                                                     [0., 0., -1.]])),
                                      np.array([0., 0., 0.111]) * self._gripper_size)

        self._gripper_parent_index = 4

        # define driver joint; the follower joints need to satisfy constraints when grasping
        # self._finger_rotation = 1.868
        # finger1_joint_ids = [16, 17, 18, 19]  # thumb, ('12', '13', '14', '15',)
        # finger2_joint_ids = [1, 2, 3, 4]  # index finger, ('1', '0', '2', '3')
        # finger3_joint_ids = [6, 7, 8, 9]  # middle finger, ('5', '4', '6', '7')
        # finger4_joint_ids = [11, 12, 13, 14]  # ring finger, ('9', '8', '10', '11')
        # self._finger_joint_ids = finger1_joint_ids + finger2_joint_ids + finger3_joint_ids + finger4_joint_ids
        # self._driver_joint_id = 6
        # self._follower_joint_ids = [1, 11, 18]
        # self._fixed_finger_joint_ids = [i for i in self._finger_joint_ids if i not in
        #                                 [self._driver_joint_id] + self._follower_joint_ids]
        # # [16, 17, 19, 2, 3, 4, 7, 8, 9, 12, 13, 14]
        self._finger_rotation = 1.868
        finger1_joint_ids = [12, 13, 14, 15]  # thumb, ('12', '13', '14', '15',)
        finger2_joint_ids = [0, 1, 2, 3]  # index finger, ('1', '0', '2', '3')
        finger3_joint_ids = [4, 5, 6, 7]  # middle finger, ('5', '4', '6', '7')
        finger4_joint_ids = [8, 9, 10, 11]  # ring finger, ('9', '8', '10', '11')
        self._finger_joint_ids = finger1_joint_ids + finger2_joint_ids + finger3_joint_ids + finger4_joint_ids
        self._driver_joint_id = 4
        self._follower_joint_ids = [0, 8, 14]
        self._fixed_finger_joint_ids = [i for i in self._finger_joint_ids if i not in
                                        [self._driver_joint_id] + self._follower_joint_ids]
        # [12, 13, 15, 0, 2, 3, 5, 6, 7, 8, 10, 11]

        self._fixed_finger_joint_position = [1.868, 0, 0.384, 0, 0.384, 0, 0, 0.384, 0, 0, 0.384, 0]

        self._palm_joint_ids = [16]
        self._joint_lower = 0.785
        self._joint_upper = 1.571
        # define force and speed (movement of mount)
        self._force = 1000
        self._speed = 2  # 0.005
        # define force and speed (grasping)
        self._grasp_force = 70
        self._grasp_speed = 2
    ...
    def reset(self, T_world_tcp):
        ...
        for id, pos in zip(self._fixed_finger_joint_ids, self._fixed_finger_joint_position):
            self.world.p.resetJointState(self.body.uid, id + self._gripper_parent_index + 1, pos)
        self.fix_joints(range(self.world.p.getNumJoints(self.body.uid)))
        # self.fix_joints(range(self._gripper_parent_index + 1))
        # self.move(1.0)
        self.move(1)
        # time.sleep(100)
        # self.joint1 = self.body.joints["5"]
        # self.joint2 = self.body.joints["1"]
        # self.joint3 = self.body.joints["9"]
        # self.joint4 = self.body.joints["14"]
        #
        # self.traced_link = self.body.links["base"]
        ...
        self.traced_link = self.body.links["palm_lower"]
    ...
    def read(self):
        ratio = ((self.joint1.get_position() + self.joint2.get_position() + self.joint3.get_position() +
                  (self.joint4.get_position() + self._joint_upper) -  4 * self._joint_lower) /
                 ((self._joint_upper - self._joint_lower) * 4))
        return 1 - ratio
    ...
    def step_constraints(self):
        ...
        #     self.world.p.POSITION_CONTROL,
        #     targetPositions=[self._finger_rotation],
        #     forces=[self._grasp_force],
        #     positionGains=[0.000001]
        # )
        # self.world.p.setJointMotorControlArray(
        #     self.body.uid,
        #     [i + self._gripper_parent_index + 1 for i in self._fixed_finger_joint_ids],
        #     self.world.p.POSITION_CONTROL,
        #     targetPositions=self._fixed_finger_joint_position,
        #     forces=[self._grasp_force] * len(self._fixed_finger_joint_position),
        #     positionGains=[0.00001] * len(self._fixed_finger_joint_position)
        # )
        ...
        self.world.p.setJointMotorControlArray(
            ...
            self.world.p.POSITION_CONTROL,
            targetPositions=[pos, pos, pos - 1.571],
...
if __name__ == '__main__':
    ...
    gripper = GripperLeapHandRight(world)
    # T_world_grasp = Transform(Rotation.from_euler('XYZ', [-np.pi, np.pi / 2., 0.]),
    #                           [0., -0.06, 0.])#gripper.finger_depth_close])
    T_world_grasp = Transform(Rotation.from_euler('XYZ', [-np.pi, 0., 0.]),
                              [0., 0., 0.])  #gripper.finger_depth_close])
    ...
    # T_world_trlink = gripper.body.links['base'].get_pose()  # at this moment, world = tcp
    ...
    # T_world_trlink = gripper.body.links['base'].get_pose()
