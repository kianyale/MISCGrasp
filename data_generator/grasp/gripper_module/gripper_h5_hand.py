...
class GripperH5Hand(GripperBase):
    """
    Initialization of H5 hand
    """
    name = 'h5_hand'
    def __init__(self, world, gripper_size=1.):
        self.world = world
        self._gripper_size = gripper_size
        self.urdf_path = Path("/media/yons/6d379145-c1d8-430f-9056-7777219c83a8/MISCGrasp/data/assets/gripper/h5_hand/combined_mount_h5_hand.urdf")
        self.max_opening_width = 0.07 * self._gripper_size
        self.finger_depth = 0.068 * self._gripper_size
        self.finger_depth_init = 0.084 * self._gripper_size
        ...
        self.T_tcp_trlink = Transform(Rotation.from_matrix(np.array([[-1., 0., 0.],
                                                                     [0., 1., 0.],
                                                                     [0., 0., -1.]])),
                                      np.array([0., 0., -0.0139]) * self._gripper_size)
    ...
    def reset(self, T_world_tcp):
        ...
        self.joint1 = self.body.joints["left_link_joint"]
        self.joint2 = self.body.joints["right_link_joint"]
    ...
    def move_tcp_xyz(self, target, eef_step=0.002, abort_on_contact=True, rotation_angle=0.):
        ...
        target_virtual = T_body_target * Transform(Rotation.identity(), [0., 0., self.finger_depth_init])  # NOTE: finger_depth_init!
...
if __name__ == '__main__':
    ...
    gripper = GripperH5Hand(world)

    T_world_grasp = Transform(Rotation.from_euler('XYZ', [-np.pi, np.pi / 3, 0.]), [0., 0., 0.])
    ...
    # diff = gripper.link1.get_pose().translation - gripper.link2.get_pose().translation
    # width = np.linalg.norm(diff)
    # print(width)
    T_temp = Transform(Rotation.identity(), [0., 0., 0.])
    # gripper.reset(T_temp)
    T_world_trlink = gripper.body.links['base_link'].get_pose()  # at this moment, world = tcp
