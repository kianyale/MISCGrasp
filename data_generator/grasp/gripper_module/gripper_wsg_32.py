...
class GripperWSG32(GripperBase):
    """
    Initialization of WSG32 2 finger gripper
    """
    name = 'wsg_32'
    ...
    def __init__(self, world, gripper_size=1.):
        ...
        self.urdf_path = Path("/media/yons/6d379145-c1d8-430f-9056-7777219c83a8/MISCGrasp/data/assets/gripper/wsg_32/combined_mount_wsg_32.urdf")
        self.max_opening_width = 0.056 * self._gripper_size
        self.finger_depth = 0.075 * self._gripper_size
        self._finger_close_distance = 0.001 * self._gripper_size
        ...
        self.T_tcp_trlink = Transform(Rotation.from_matrix(np.array([[1., 0., 0.],
                                                                     [0., 1., 0.],
                                                                     [0., 0., 1.]])),
                                      np.array([0., -0.0017, -0.0344]) * self._gripper_size)
    ...
    def reset(self, T_world_tcp):
        ...
        self.world.add_constraint(
            self.body,
            self.body.links["left_universal_finger_link"],
            self.body,
            self.body.links["right_universal_finger_link"],
            pybullet.JOINT_GEAR,
            [1.0, 0.0, 0.0],
            Transform.identity(),
            Transform.identity(),
        ...
        self.joint1 = self.body.joints["wsg_50_gripper_base_joint_gripper_left"]
        self.joint2 = self.body.joints["wsg_50_gripper_base_joint_gripper_right"]
        self.traced_link = self.body.links["hand_link"]
    ...
    def read(self):
        width = abs(self.joint1.get_position()) + self.joint2.get_position()
        return width / self.max_opening_width

    def open(self, open_scale):
        joint_ids = [i + self._gripper_parent_index + 1 for i in self._moving_joint_ids]
        ...
        self.world.p.setJointMotorControlArray(
            ...
            forces=[self._grasp_force] * len(joint_ids)
        )

    def close(self):
        self.world.p.setJointMotorControl2(
            self.body.uid,
            self._moving_joint_ids[0] + self._gripper_parent_index + 1,
    ...
    def step_constraints(self):
        pos = self.world.p.getJointState(self.body.uid, self._moving_joint_ids[0] + self._gripper_parent_index + 1)[0]
        self.world.p.setJointMotorControl2(
            self.body.uid,
            self._moving_joint_ids[1] + self._gripper_parent_index + 1,
            self.world.p.POSITION_CONTROL,
            targetPosition=-pos,
            force=self._grasp_force,
...
if __name__ == '__main__':
    ...
    gripper = GripperWSG32(world)
    T_world_grasp = Transform(Rotation.from_euler('XYZ', [-np.pi, np.pi / 2, 0.]), [0., -0.028, 0.])
    ...
    print(gripper.read())
    T_world_trlink = gripper.body.links['hand_link'].get_pose()
